...
| `:MdRender textsize [on\|off\|toggle]` | **[実験的]** Kitty text sizing protocol で `#` / `##` の見出しを拡大 |
...
> **実験的機能です。** 新しく追加された opt-in の機能で、Kitty 専用です。UX が変わったり、機能ごと取り下げられる可能性があります。問題や違和感があればぜひお知らせください。

Kitty 0.40 で追加された [text sizing protocol](https://sw.kovidgoyal.net/kitty/text-sizing-protocol/) (OSC 66) は、テキストを基準フォントサイズの整数倍で描画します。md-render はこれを使って `#` と `##` の見出しを 2 倍の大きさで表示できます:

```lua
require("md-render.text_size").setup { enabled = true }
```

実行中の切り替えは `:MdRender textsize` で行います。

どの見出しを拡大するかは固定です。`#` と `##` を `s=2` で拡大し、それ以外は等倍のままです。`s=2` は高さだけでなく幅も 2 倍になるため、これより深いレベルはプレビューウィンドウに収まらなくなります。長い見出しは通常の半分の幅で折り返すので `#` / `##` は必ず拡大され、折り返した行はそれぞれ 2 行ぶんのブロックになります。
...
- **Kitty 0.40 以降でのみ動作します。** 対応判定は端末に自己申告させる方法 (XTVERSION) で行い、肯定的な応答があった場合のみ有効になります。これは意図的に厳しくしてあり、OSC 66 を**ペイロードのテキストごと**捨てる端末があるためです(等倍で表示されるのではなく、見出しが消えてしまう)。
- 見出し内のインライン書式(インラインコード・リンク・`==highlight==`)は拡大中は色が失われます。
- 2 行目がウィンドウの外にはみ出す見出しは、スクロールして見えるようになるまで等倍のままです。
- md-render が観測できない再描画(他のプラグインによる強制再描画、メッセージやポップアップの重なり)が起きた場合、次のスクロールかカーソル移動まで見出しは等倍のままになります。
- レイアウトが変わるたびに、前回の拡大テキストを消すために画面全体を再描画します。そのぶんスクロールのコストは通常より高くなります。
